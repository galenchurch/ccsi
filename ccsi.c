...
size_t output_len_calc(size_t len){
    ...
    bits = (words * 16) + words + STOP_HOLD_BITLEN; //num words * 16bits + words (valid bit) + 18 stop bits
    array_len = (bits % 8) > 0 ? 1 : 0;
    array_len += (bits / 8);
...
int8_t ccsi_packet_gen(uint8_t *in, size_t len_in, uint8_t *out, size_t len_out, uint8_t fix_valid){
    ...
    for(i = 0; i < len_in; i=i+2){
        ...
        if (fix_valid == 1){
            ...
            out[j+2] = (mask_valid & (~(in[i+1]) << (7-shift)))  | (in[i+1] << (8-shift) );
